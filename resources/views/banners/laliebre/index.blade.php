                    ...
                    <table class="table">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td>{{ $item->id}}</td>
                                <td style="width: 30%">
                                  <img src="{{url('img/bannerLiebre/'.$item->image)}}" class="img-responsive img-fluid" style="width:30%; height:20%"alt="">
                                </td>
                                <td>
                                  <a href="{{ route('bannerLiebre.edit',$item->id)}}">Editar</a> </td>
                                <td>
                                    <form class="user"  action="{{route('bannerLiebre.delete', $item->id)}}" method="post">
                                        {{ method_field('delete') }}
                                        {{csrf_field()}}
    ...
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        ...
        <div class="modal-body">
          <form action="{{ route('bannerLiebre.store') }}" id="" method="post" enctype="multipart/form-data">
            @csrf
